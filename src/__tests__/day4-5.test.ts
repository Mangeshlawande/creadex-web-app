...
function makeAudit(overrides: Partial<AuditResult> = {}): AuditResult {
  return {
    ...
    formData: { teamSize: 3, useCase: "coding", tools: [] },
    toolResults: [
      {
        toolEntry: { toolId: "cursor", planId: "cursor_business", seats: 3, monthlySpend: 120 },
        toolName: "Cursor",
        planName: "Business",
        ...
        monthlySavings: 60,
        annualSavings: 720,
        reasoning: "Teams under 5 don't need Cursor Business SSO/admin features.",
      },
    ],
...
describe("buildFallbackSummary", () => {
  ...
  test("returns positive messaging for optimal audits", () => {
    const audit = makeAudit({
      ...
      toolResults: [
        {
          toolEntry: { toolId: "cursor", planId: "cursor_pro", seats: 1, monthlySpend: 20 },
          toolName: "Cursor",
          planName: "Pro",
